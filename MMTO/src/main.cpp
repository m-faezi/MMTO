...
namespace hg {
    ...
    namespace tree_fusion_internal {
        ...
        auto plot_map(
            ...
            vector<vector<vector<float>>> merger;
            vector<vector<index_t>> adj_lists;
            vector<array_1d<index_t>> areas;
            array_2d<array_1d<index_t>> ses = xt::empty<array_1d<index_t>>({ntrees, ntrees});
            ...
            for (
                ...
                for (index_t n: leaves_to_root_iterator(**ti, leaves_it::include, root_it::exclude)) {
                    ...
                    if (!((**xi)[n] < 0)){
                        vector<float> object;
                        object.push_back(i+1);
                        object.push_back((**ai)[n]);
                        object.push_back((**xi)[n]);
                        object.push_back((**yi)[n]);
                        object.push_back((**mi)[n]);
                        vector<vector<float>> merge_self;
                        merge_self.push_back(object);
                        merge_self.push_back(object);
                        ...
                        if (
                            ...
                            if (
                                ...
                                if (cosine_similarity(X, Y, 4) > 0.5){
                                    ...
                                    host.push_back((**xj)[ses_ij_n]);
                                    host.push_back((**yj)[ses_ij_n]);
                                    host.push_back((**mj)[ses_ij_n]);
                                    vector<float> merged;
            ...
            return pybind11::array(pybind11::cast(merger));
        ...
        PYBIND11_MODULE(mmto, m)
        {
            ...
            m.def("plot_map", plot_map, "Plot map function.");
